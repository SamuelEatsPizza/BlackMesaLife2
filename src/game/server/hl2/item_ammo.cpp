...
public:
	...
	{
		Precache();
		SetModel("models/weapons/w_9mmclip.mdl");
		BaseClass::Spawn();
	}
	void Precache(void)
	{
		PrecacheModel("models/weapons/w_9mmclip.mdl");
	}
	bool MyTouch(CBasePlayer* pPlayer)
	{
		if (ITEM_GiveAmmo(pPlayer, SIZE_AMMO_PISTOL, "9mm"))
		{
			if (g_pGameRules->ItemShouldRespawn(this) == GR_ITEM_RESPAWN_NO)
			{
				UTIL_Remove(this);
...
LINK_ENTITY_TO_CLASS(item_ammo_glock, CItem_Box9mmRounds);
